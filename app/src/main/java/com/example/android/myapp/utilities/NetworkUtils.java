...
public final class NetworkUtils {
 ...
 /*       if (SunshinePreferences.isLocationLatLonAvailable(context)) {
    ...
    private static URL buildUrlWithLatitudeLongitude(Double latitude, Double longitude) {
        Uri weatherQueryUri = Uri.parse("127.0.0.1:8080").buildUpon()
    ...
    private static URL buildUrlWithLocationQuery(String locationQuery) {
        Uri weatherQueryUri = Uri.parse(MYURL+"/retrieveall").buildUpon()
/*                .appendQueryParameter(QUERY_PARAM, locationQuery)
        ...
        try {
            URL weatherQueryUrl = new URL(weatherQueryUri.toString());
            Log.v("taaaag", "URL: " + weatherQueryUrl);
            return weatherQueryUrl;
        } catch (MalformedURLException e) {
            e.printStackTrace();
            return null;
    ...
    public static URL buildUrlfullsync() {
        Uri weatherQueryUri = Uri.parse(MYURL+"/retrieveallcs").buildUpon()
/*                .appendQueryParameter(QUERY_PARAM, locationQuery)
                .appendQueryParameter(FORMAT_PARAM, format)
                .appendQueryParameter(UNITS_PARAM, units)
                .appendQueryParameter(DAYS_PARAM, Integer.toString(numDays))*/
        ...
        try {
            URL weatherQueryUrl = new URL(weatherQueryUri.toString());
            Log.v("taaaaag", "URL: " + weatherQueryUrl);
            return weatherQueryUrl;
        } catch (MalformedURLException e) {
            e.printStackTrace();
            return null;
